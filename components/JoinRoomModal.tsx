...
interface JoinRoomModalProps {
    isOpen: boolean;
    onClose: () => void;
    onJoin: (roomId: string, password?: string) => 'joined' | 'needs_password' | 'invalid_password' | 'not_found' | 'already_joined';
}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={handleClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="invite-code" className="text-sm font-medium text-gray-600 dark:text-gray-400">Invite Code</label>
                        <input 
                            id="invite-code"
                            type="text" 
                            value={roomId} 
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-t dark:border-gray-700 flex justify-end">
                     <button onClick={handleJoin} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700">
                        <IconLogin className="w-5 h-5"/> {step === 'enter_password' ? 'Enter Room' : 'Join Room'}
                    </button>
                </div>
            </div>
        </div>
