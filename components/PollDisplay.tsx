...
const PollDisplay: React.FC<PollDisplayProps> = ({
  ...
  const userVote = useMemo(
    () => poll.options.find((opt) => opt.votes.includes(currentUser.id)),
    ...
    <div className={`my-2 w-full max-w-sm sm:w-80 md:w-96 lg:w-[32rem] ${textColor}`}>
      ...
      <div className="space-y-2">
        {poll.options.map((option) => {
          ...
          const baseBorder = isCurrentUserMessage
            ? 'border-white/50'
            : 'border-gray-300 dark:border-gray-600';
          const hoverBorder = isCurrentUserMessage
            ? 'hover:border-white/80'
            : 'hover:border-primary-500 dark:hover:border-primary-400';
          const votedRing = isCurrentUserMessage ? 'ring-white/80' : 'ring-primary-500';
          const progressBg = isCurrentUserMessage
            ? 'bg-white/25'
            : 'bg-primary-100 dark:bg-primary-900/50';
          const optionTextColor = isCurrentUserMessage
            ? 'text-white'
            : 'text-gray-800 dark:text-gray-100';
          const voteCountColor = isCurrentUserMessage
            ? 'text-white/90'
            ...
            <div key={option.id} className="relative">
              ...
              >
                ...
                <div className="relative z-10 flex justify-between items-center">
                  <span className={`font-medium text-sm sm:text-base ${optionTextColor}`}>
                    {option.text}
                  </span>
                  {userVote && (
                    <span className={`text-xs sm:text-sm font-semibold ${voteCountColor}`}>
      ...
      <p
        className={`text-xs text-right mt-2 ${isCurrentUserMessage ? 'text-white/80' : 'text-gray-500 dark:text-gray-400'}`}
      >
