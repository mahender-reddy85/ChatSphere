...
const VideoCallModal: React.FC<VideoCallModalProps> = ({ room, currentUser, onLeaveCall }) => {
    ...
    if (isScreenSharing) {
      ...
      try {
        ...
        if (err instanceof DOMException && err.name === 'NotAllowedError') {
          alert(
            'Screen sharing permission was denied. To share your screen, you must grant permission when prompted.'
          );
        } else {
          alert(
            'Could not start screen sharing. Please ensure your browser supports this feature and try again.'
          );
        }
      }
    }
  ...
  const ParticipantTile: React.FC<{ participant: User; isLocal: boolean; isThumb: boolean }> = ({
    participant,
    isLocal,
    isThumb,
  }) => (
    <div
      className={`relative bg-gray-800 rounded-lg overflow-hidden ${isThumb ? 'w-full aspect-video' : 'w-full h-full'}`}
    >
      {isLocal ? (
        <>
          <video
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex flex-col p-4">
      <div
        className={`flex-1 flex gap-4 overflow-hidden ${isScreenSharing ? 'flex-col md:flex-row' : ''}`}
      >
      ...
      <div className="flex-shrink-0 flex justify-center items-center pt-4">
        <div className="flex items-center gap-4 bg-gray-800 p-3 rounded-full">
          <button
            onClick={toggleMic}
            className={`p-3 rounded-full ${isMicMuted ? 'bg-red-600' : 'bg-gray-600'} text-white`}
          >
            {isMicMuted ? <IconMicOff className="w-6 h-6" /> : <IconMic className="w-6 h-6" />}
          </button>
          <button
            onClick={toggleCamera}
            className={`p-3 rounded-full ${isCameraOff ? 'bg-red-600' : 'bg-gray-600'} text-white`}
          >
