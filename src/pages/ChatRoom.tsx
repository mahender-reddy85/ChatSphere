...
import { ArrowLeft, Send, Copy, Link, Users, Circle, Timer, DoorOpen, Settings, QrCode, Trash2, ArrowUp } from "lucide-react";
...
const ChatRoom = () => {
  ...
  // Removed playSound hook usage as per request to disable sound features
  ...
  useEffect(() => {
    ...
    return () => unsub();
  }, [roomId, user?.uid, navigate, quotaExceeded]);

  // Temporary room timer calculation
  useEffect(() => {
    if (room?.chatMode !== "temporary" || !room.createdAt || !room.autoDeleteMinutes) return;

    const calculateTimeLeft = () => {
      const createdAtMs = room.createdAt.toMillis?.() ?? Date.now();
      const expiryTime = createdAtMs + room.autoDeleteMinutes! * 60 * 1000;
      const remainingMs = Math.max(0, expiryTime - Date.now());
      
      if (remainingMs === 0) {
        setTimeLeft("00:00");
        if (roomId) deleteDoc(doc(db, "rooms", roomId)).catch(() => {});
        return;
      }
  ...
  }, [room?.createdAt, room?.chatMode, room?.autoDeleteMinutes, roomId]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      setMessageVisibility(prev => {
        ...
        let changed = false;
        msgs.forEach(msg => {
          if (!updated[msg.senderId]) {
            updated[msg.senderId] = { showAvatar: true, showName: false };
            changed = true;
          }
        });
  ...
  const handleLeaveRoom = async () => {
    ...
    try {
      ...
      // If the creator leaves, delete the entire room
      if (user.uid === room.createdBy) {
        ...
        batch.delete(roomRef);
        await batch.commit();
        toast.success("Room closed (Creator left)");
        navigate("/home");
        return;
      }
      ...
      if (newParticipants.length === 0) {
        // Delete all messages then delete room (fallback for when creator is somehow not present)
        const messagesSnap = await getDocs(collection(db, "rooms", roomId, "messages"));
        const batch = writeBatch(db);
        messagesSnap.docs.forEach((d) => batch.delete(d.ref));
        batch.delete(roomRef);
    ...
    <div className="flex h-screen max-h-screen flex-col mobile-full-height">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {room.chatMode === "temporary" && (
              <div className="flex items-center gap-1 px-2 py-0.5 rounded bg-muted/50 border border-border/50">
      ...
      >
        ...
        {messages.map((msg, index) => {
          const visibility = messageVisibility[msg.senderId] || { showAvatar: true, showName: false };
